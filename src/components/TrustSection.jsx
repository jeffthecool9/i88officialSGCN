...
const payments = [
  ...
  { name: '网上银行',  color: 'text-ice/70' },
  { name: '加密货币',  color: 'text-yellow-400' },
]
...
const trust = [
  { Icon: IconLock,    label: 'SSL 加密' },
  { Icon: IconBolt,    label: '快速提款' },
  { Icon: IconShield,  label: '负责任博彩' },
  { Icon: IconSupport, label: '全天候客服' },
]
  ...
  return (
    <section className="py-12 px-6">
      ...
      <div className="max-w-md mx-auto">
        ...
        >
          <h2 className="font-display text-3xl text-ice tracking-widest">可信 · 安全 · 可靠</h2>
          <p className="font-body text-ice/65 text-sm mt-1">持牌监管 · 马来西亚 &amp; 新加坡</p>
        </motion.div>
        ...
        >
          {payments.map((p) => (
            <div key={p.name} className="px-4 py-2 rounded-lg bg-surface/50 border border-white/8">
              <span className={`font-body font-semibold text-sm tracking-wide ${p.color}`}>
                {p.name}
              </span>
            </div>
          ))}
        ...
        >
          {trust.map((b) => (
            ...
            >
              ...
              <span className="font-body text-ice/80 text-sm tracking-wide">{b.label}</span>
            </div>
          ))}
        </motion.div>
        ...
        <motion.p
          className="font-body text-ice/50 text-xs text-center leading-relaxed"
          initial={{ opacity: 0 }}
          whileInView={{ opacity: 1 }}
          viewport={{ once: true }}
          transition={{ duration: 0.6, delay: 0.25 }}
        >
          仅限18岁以上。请负责任地博彩。i88 支持马来西亚与新加坡的安全博彩。
          新加坡 <span className="text-brand/40">1800-6-668-668</span> · 马来西亚 <span className="text-brand/40">1-800-88-3151</span>
        </motion.p>
      </div>
    </section>
  )
